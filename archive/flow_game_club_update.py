...
class FlowGameClubUpdate:
    def __init__(self):
        ...
        # 必要info
        self.login_info = LoginInfo.SITE_PATTERNS.value["LGRAM"]
        self.update_info = UpdateInfo.LGRAM.value
    ...
    def _get_title_link(self, max_update: int = 15):
        ...
        while len(all_title_link) < max_update:
            ...
            # 一度出品中のある要素に絞り込みをする。
            item_rows = self.element.getElements(
                by=self.update_info["ITEM_ROWS_BY"],
                value=self.update_info["ITEM_ROWS_VALUE"],
            )
            ...
            if item_rows:
                for item_row in item_rows:
                    ...
                    status_element = self.element.filterElements(
                        parentElement=item_row,
                        by=self.update_info["ITEM_ROW_BY"],
                        value=self.update_info["ITEM_ROW_VALUE"],
                    )
                    self.logger.debug(f"status_element: {status_element}")

                    # 出品中のステータスがあるかどうかを真偽値で返す（any）
                    active_status = any(
                        status.text == "出品中" for status in status_element
                    )
                    self.logger.debug(f"active_status: {active_status}")

                    # 出品中のステータスを確認できたもの
                    if active_status:
                        link_element = self.element.filterElement(
                            parentElement=item_row,
                            by=self.update_info["TITLE_LINK_BY"],
                            value=self.update_info["TITLE_LINK_VALUE"],
                        )
                        self.logger.debug(f"link_element: {link_element}")
                        element_link = link_element.get_attribute(
                            "href"
                        )  # リンクを取得
    ...
    def _update_all_process(self, link_list: List):
        if link_list:
            ...
            # 更新ボタンが押せなくなった処理
                ...
                self.logger.info(
                    f"更新の上限に達しました: 実施回数 {i + 1}回、Update実施"
                )
                return self.chrome.quit()

            except Exception as e:
                self.logger.error(
                    f"アップデート実施中になにかしらのエラー発生(止めずにそのまま処理を続行): {e}"
                )
                return self.chrome.quit()

        else:
            self.logger.warning(
                f"【更新処理スキップ】ウォッチリストに出品中のものが登録されていません。"
            )
            return self.chrome.quit()
        ...
        while count < max_count:
            if not disable_element_bool:
                ...
                try:
                    ...
                    self.logger.info(
                        f"更新の上限に達しました: 実施回数 {count}回、Update実施"
                    )
                    break
...
if __name__ == "__main__":
    id_text = LoginInfo.SITE_PATTERNS.value["LGRAM"]["ID_TEXT"]
    pass_text = LoginInfo.SITE_PATTERNS.value["LGRAM"]["PASS_TEXT"]
