    ...
    def process(self, gss_info: Dict, err_datetime_cell: str, err_cmt_cell: str, max_count: int=3):
        ...
        while retry_count < max_count:
            try:
                ...
                self.logger.warning(f'{self.__class__.__name__} エラー発生、リトライ実施: {retry_count + 1}/{max_count}')
                time.sleep(1)  # 少し待って再取得
                retry_count += 1

            except NewConnectionError as e:
                self.logger.warning(f'{self.__class__.__name__} 以前にエラーがありChromeなし: スキップ実施')
                raise

            except Exception as e:
                self.logger.error(f'{self.__class__.__name__} 処理中にエラー発生中: {e}')
                error_comment = "【タグ管理】未分類以外に「対応中」がある可能性大。"
